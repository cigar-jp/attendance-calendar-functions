...
function addEventsFromSpreadsheet() {
  ...
  if (!targetName || !calendarId || !spreadsheetId) {
    Logger.log("プロジェクトのプロパティに TARGET_NAME または CALENDAR_ID または SPREADSHEET_ID が設定されていません。");
    return;
  }
  ...
  filteredData.forEach((row, index) => {
    ...
    Logger.log("行 " + (index+1) + "：スケジュール雛形ID = " + scheduleTemplateId);
    ...
    if (scheduleTemplateId === "0") {
      ...
      title = "公休";
      Logger.log("行 " + (index+1) + "：スケジュール雛形IDが0のためタイトルは「公休」");
    } else {
      ...
      if (startTimeStr && endTimeStr) {
        ...
        Logger.log("行 " + (index+1) + "：出勤 " + startTimeStr + " → " + startDecimal + ", 退勤 " + endTimeStr + " → " + endDecimal + " によりタイトル作成: " + title);
      } else {
        Logger.log("行 " + (index+1) + "：出勤予定時刻または退勤予定時刻が不足しているためスキップ");
        return;
      }
    }
    ...
    Logger.log("行 " + (index+1) + "：カレンダーにイベントを作成しました。日付: " + eventDate.toDateString() + " / タイトル: " + title);
  });
}
